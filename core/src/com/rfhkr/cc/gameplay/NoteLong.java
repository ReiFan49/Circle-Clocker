...
class NoteLong extends NoteBasic implements EarlyJudgable,LateJudgable {
	...
	public void draw(final SpriteBatch batch) {
		...
		// only draw this if
		if(fPro2.apply(0) >= 0.0f && Gameplay.now().getElapsed() < limhgh) {
			if(fPro2.apply(0) < 0.5f) {
				...
				batch.draw(noteSprite.get(1),fX.apply(1),fY.apply(1), 0,32,240*(1-pcOFFSET)*limprx,64,1,1,drawA,0,0,noteSprite.get(1).getWidth(),64,false,true);
